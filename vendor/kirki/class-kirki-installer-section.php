			...
			class Kirki_Installer_Section extends WP_Customize_Section {
				...
				protected function install_button() {
					...
					<p style="text-align:left;margin-top:0;">
						<?php esc_html_e( 'Please install the \'Kirki\' plugin to take full advantage of this theme\'s customizer capabilities', 'crescent' ); ?>
					</p>
					<a class="install-now button-primary button" data-slug="kirki" href="<?php echo esc_url_raw( $this->get_plugin_install_url() ); ?>" aria-label="<?php esc_attr_e( 'Install Kirki Toolkit now', 'crescent' ); ?>" data-name="Kirki Toolkit">
						<?php esc_html_e( 'Install Now', 'crescent' ); ?>
					</a>
					<?php
				}
				...
				protected function activate_button() {
					?>
					<p style="text-align:left;margin-top:0;">
						<?php esc_html_e( 'You have installed Kirki. Activate it to take advantage of this theme\'s features in the customizer.', 'crescent' ); ?>
					</p>
					<a class="activate-now button-primary button" data-slug="kirki" href="<?php echo esc_url_raw( self_admin_url( 'plugins.php' ) ); ?>" aria-label="<?php esc_attr_e( 'Activate Kirki Toolkit now', 'crescent' ); ?>" data-name="Kirki Toolkit">
						<?php esc_html_e( 'Activate Now', 'crescent' ); ?>
					</a>
					<?php
				}
				...
				protected function dismiss_button() {
					...
					$show_confirm = true;
					?>
					<a class="kirki-installer-dismiss button-secondary button" data-slug="kirki" href="#" aria-label="<?php esc_attr_e( 'Don\'t show this again', 'crescent' ); ?>" data-name="<?php esc_attr_e( 'Dismiss', 'crescent' ); ?>">
						<?php esc_html_e( 'Don\'t show this again', 'crescent' ); ?>
					</a>
					...
					jQuery( document ).ready( function() {
						jQuery( '.kirki-installer-dismiss' ).on( 'click', function( event ) {
							...
							<?php if ( $show_confirm ) : ?>
								if ( ! confirm( '<?php esc_html_e( 'Are you sure? Dismissing this message will hide the installation recommendation and you will have to manually install and activate the plugin in the future.', 'crescent' ); ?>' ) ) {
									return;
								}
							<?php endif; ?>
