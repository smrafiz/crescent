...
class Crescent_Breadcrumbs {
	...
	public function get_breadcrumbs( array $args ) {
		...
		$defaults = array(
			...
			'home_text'             		=> sprintf( '<i class="fa fa-home"></i> <span>%s</span>', esc_attr__( 'Home', 'crescent' ) ),
			'cat_archive_prefix'     		=> esc_attr__( 'Category:&nbsp;', 'crescent' ),
			'tag_archive_prefix'     		=> esc_attr__( 'Tag:&nbsp;', 'crescent' ),
			'search_prefix'          		=> esc_attr__( 'Search query for:&nbsp;', 'crescent' ),
			'error_prefix'           		=> esc_attr__( '404 - Page not found', 'crescent' ),
		);

		$defaults     						= apply_filters( 'crescent_breadcrumbs_defaults', $defaults );
	...
	private function start_breadcrumb_rendering() {
		...
		// Woocommerce support.
		if ( class_exists( 'WooCommerce' ) && ( ( crescent_inside_woocommerce() && is_archive() && ! is_shop() ) || is_cart() || is_checkout() || is_account_page() ) ) {
		...
		if ( is_singular() ) {
			...
			} else if ( ( is_tax() || is_tag() || is_category() || is_date() || is_author() ) && $this->display_post_type_archive && ! crescent_inside_woocommerce() ) {
				$this->render_markup .= $this->get_post_type_archive();
			}
			...
			if ( is_post_type_archive() ) {
				...
				// 404 Page.
				if ( crescent_inside_tribe_event() || ( is_post_type_archive( 'tribe_events' ) || ( crescent_inside_tribe_event() && is_archive() ) ) ) {
	...
	private function get_terms() {
		...
		if ( 'post' == $this->post->post_type ) {
			$taxonomy = 'category';
		} elseif ( 'product' == $this->post->post_type && class_exists( 'WooCommerce' ) && crescent_inside_woocommerce() ) {
	...
	private function get_woocommerce_page( $linked = true ) {
		...
		if ( is_object( $post_type_object ) && class_exists( 'WooCommerce' ) && ( crescent_inside_woocommerce() || is_cart() || is_checkout() || is_account_page() ) ) {
	...
	private function get_breadcrumb_trail_markup( $object_type = '' ) {
		...
		switch ( $object_type ) {
			...
			case 'year':
				...
				$title = esc_html__( 'Year: ', 'crescent' ) . $year;
				break;
			case 'month':
				...
				$title = esc_html__( 'Month: ', 'crescent' ) . $wp_locale->get_month( $monthnum );
				break;
			case 'day':
				...
				$title = esc_html__( 'Day: ', 'crescent' ) . $day;
				break;
			case 'author':
				...
				$title = esc_html__( 'Articles Posted by&nbsp;', 'crescent' ) . $user->display_name;
				break;
			case 'search':
