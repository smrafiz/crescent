...
class Crescent_Pagination {
    ...
    private function screen_reader_text() {
        return '<h2 class="screen-reader-text">' . esc_html__( 'Post navigation', 'crescent' ) . '</h2>';
    }
    ...
    private function nav_wrapper() {
        $this->render_nav = '<nav id="post-navigation" class="navigation post-navigation" aria-label="' . esc_html__( 'Post navigation', 'crescent' ) . '">' . $this->render_nav . '</nav><!-- #post-navigation -->';
    ...
    private function output_nav( string $type ) {
        echo apply_filters( "crescent_{$type}_nav", $this->render_nav ); // WPCS: XSS ok.
    }
